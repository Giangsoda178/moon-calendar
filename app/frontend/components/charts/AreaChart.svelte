<!--
  @component AreaChart
  Multi-series area chart with Svelte 5 snippet API.
  ...
  ```svelte
  <AreaChart {data} x="month" series={['desktop', 'mobile']} />
  ```

  @example With legend on the right
  ```svelte
  <AreaChart {data} x="month" series={['desktop', 'mobile']} legendPosition="right" />
  ```
...
<script lang="ts" generics="T extends object">
  ...
  import { AreaChart as LayerAreaChart, Tooltip } from "layerchart"
    ...
    <LayerAreaChart
      ...
      props={{
        area: {
          ...
          line: showLine ? { class: "stroke-2" } : { class: "stroke-0" },
          motion: "tween",
        },
        xAxis: {},
        yAxis: { format: (d: number) => d.toLocaleString() },
        highlight: {
          points: { r: 4 },
        },
      }}
